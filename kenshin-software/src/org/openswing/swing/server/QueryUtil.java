 ...
 */
  ...
  private static Response getQuery(
    ...
    Statement stmt = null;// edit n.ohkubo 2014/10/01　「を含む」検索（like検索）の不具合対応（SQLがlike検索の場合、PreparedStatementではなくStatementで実行）
    String params = "";
    if (baseSQL==null)
      baseSQL =
        "SELECT "+select+" "+
    ...
    try {
      ...
      // edit n.ohkubo 2014/10/01　start　「を含む」検索（like検索）の不具合対応（SQLがlike検索の場合、PreparedStatementではなくStatementで実行）
      ResultSet rset = null;
      String modSQL = baseSQL;
      if (baseSQL.matches(".* like \\?.*") || baseSQL.matches(".* LIKE \\?.*")) {
        for(int i=0;i<values.size();i++) {
           modSQL = modSQL.replaceFirst("\\?","'"+values.get(i)+"'");
        }
        System.out.println("modSQL = "+modSQL);
        stmt = conn.createStatement();
        rset = stmt.executeQuery(modSQL);
      }
      else {
...
//            rset = pstmt.executeQuery();
            ...
            // edit n.ohkubo 2014/10/01　end　「を含む」検索（like検索）の不具合対応（SQLがlike検索の場合、PreparedStatementではなくStatementで実行）
