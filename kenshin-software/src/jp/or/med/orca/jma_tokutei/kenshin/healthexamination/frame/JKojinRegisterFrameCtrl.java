...
//		}
	 ...
	 * フレームの初期化処理
	 */
	public JKojinRegisterFrameCtrl(JKekkaListFrameData data) {
		this();
		isFromKekkaRegster = true;

		String uketukeId = data.getUketuke_id();
		setKojinDataFromDB(uketukeId);
	}
...
//	}
	...
	private boolean validateInputData() {
		...
		boolean isValid =
				...
				/* 交付日 */
				&& validatedData.setIssueDate(jTextField_IssueDate.getText())
				/* 受診券整理番号 */
				&& validatedData.setJyushinkenID(jTextField_JyushinkenID
						.getText())
				/* 有効期限 */
				&& validatedData.setLimitDate(jTextField_LimitDate.getText())
				/* 携帯電話番号 */
				&& validatedData.setMobileMail(jTextField_MobileMail.getText())
				/* 氏名（カナ） */
				&& validatedData.setNameKana(jTextField_NameKana.getText())
				/* 氏名 */
				...
				/* 性別 */
				&& validatedData.setSex(jTextField_sex.getText())
				/* 契約取りまとめ機関名 */
				...
				/* 窓口負担（基本的な健診） */
				&& validatedData.setMadoguchiKihonSyubetu(jComboBox_MadoguchiKihonSyubetu.getSelectedIndex())
				&& validatedData.setMadoguchiKihon(jTextField_MadoguchiKihon.getText())

				/* 窓口負担（詳細な健診） */
				&& validatedData.setMadoguchiSyousaiSyubetu(jComboBox_MadoguchiShousaiSyubetu.getSelectedIndex())
				&& validatedData.setMadoguchiSyousai(jTextField_MadoguchiShousai.getText())

				/* 窓口負担（追加の健診） */
				&& validatedData.setMadoguchiTsuikaSyubetu(jComboBox_MadoguchiTsuikaSyubetu.getSelectedIndex())
				&& validatedData.setMadoguchiTsuika(jTextField_MadoguchiTsuika.getText())

				/* 窓口負担（人間ドック） */
				&& validatedData.setMadoguchiDockSyubetu(jComboBox_MadoguchiDockSyubetu.getSelectedIndex())
				&& validatedData.setMadoguchiDock(jTextField_MadoguchiDock.getText())

				/* 保険者負担額上限 */
				&& validatedData.setHokenjyaJougenKihon(jTextField_KihonJougen.getText())
				&& validatedData.setHokenjyaJougenSyousai(jTextField_ShousaiJougen.getText())
				&& validatedData.setHokenjyaJougenTsuika(jTextField_TsuikaJyougen.getText())
				&& validatedData.setHokenjyaJougenDoc(jTextField_DockJougen.getText())

				/* その他の健診の負担額 */
				&& validatedData.setMadoguchiSonota(jTextField_sonotaHutangaku.getText())
	 ...
	 * 受診者情報(更新処理時の画面制御)
	 */
	private void presetKojinData(Hashtable<String, String> resultItem) {
		...
		setKojinData(resultItem);
	}

	/**
	 * 受診者情報を画面上に表示する。
	 */
	private void setKojinData(Hashtable<String, String> resultItem) {
...
//				jRadioButton_Kihon.setSelected(true);
		...
		// edit s.inoue 2009/09/30
		validatedData.setpreJyushinkenID(resultItem.get("JYUSHIN_SEIRI_NO"));
	}
	 ...
	 *            受付ID
	 */
	public void setKojinDataFromDB(String uketukeId) {

		if (!validatedData.setJyushinkenID(jTextField_JyushinkenID.getText()))
			return;
...
//			JQueryConvert.queryConvert(uketukeId);
		...
		if (result.isEmpty()) {
			...
			/* 受診者情報を画面上に表示する。 */
			this.presetKojinData(result.get(0));
		}
	}
	...
	public void setKojinDataFromShimeiKana(String kanaName) {
		...
		if (!validatedData.setNameKana(kanaName))
			return;
//		// →1件のみ 完全一致
//		// →複数件 曖昧検索
...
//		kojinDataList(resultKanaName,FLAME_TITLE_NAME_MESSAGE);
		...
		kanaQuery.append("SELECT PTNUM, JYUSHIN_SEIRI_NO, YUKOU_KIGEN, HKNJANUM, HIHOKENJYASYO_KIGOU, ");
		...
		kanaQuery.append(" MADO_FUTAN_TSUIKA, MADO_FUTAN_D_SYUBETU, MADO_FUTAN_DOC, NENDO, UKETUKE_ID, MADO_FUTAN_SONOTA,");
		kanaQuery.append(" HOKENJYA_FUTAN_KIHON, HOKENJYA_FUTAN_SYOUSAI, HOKENJYA_FUTAN_TSUIKA, HOKENJYA_FUTAN_DOC");
		kanaQuery.append(" FROM T_KOJIN WHERE KANANAME ");
...
//			validatedData.setChiban(resultItem.get("home_banti"));
	...
	public void pushedPrintButton(ActionEvent e) {
		...
		progressWindow
				.setGuidanceByKey("tokutei.jushinken.frame.progress.guidance.print1");
...
//			tmpKojin.put("KENSA_NENGAPI", Nendo);
	...
	private void showAddHokenjaNumberDialog(String inputNumber) {
		...
		if (inputNumber == null || inputNumber.isEmpty()) {
			...
			validatedData.setHokenjyaNumber(inputNumber);
...
//			jRadioButton_Kihon.setSelected(true);
	...
	private void showUnitAndSyubetsuNumToTextField(
			ExtendedTextField jTextField_SyubetsuNum,
			ExtendedTextField jTextField_Madoguchi,
			ExtendedComboBox jComboBox_MadoguchiSyubetu,
			ExtendedLabel jLabel_madoguchi_unit,
			ExtendedTextField jTextField_Jougen
			) {
...
//	}
	...
	public void focusLost(FocusEvent event) {
		...
		if (source.equals(this.jComboBox_HokenjyaNumber)) {
			String selectedItem = (String)
				this.jComboBox_HokenjyaNumber.getSelectedItem();
		...
		else if(source.equals(this.jComboBox_DaikouNumber)){
			String selectedItem = (String)
				this.jComboBox_DaikouNumber.getSelectedItem();
