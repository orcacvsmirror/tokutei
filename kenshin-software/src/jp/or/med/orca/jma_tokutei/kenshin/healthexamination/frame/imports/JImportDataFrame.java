...
public class JImportDataFrame extends JFrame implements ActionListener,ItemListener,KeyListener {
	...
	protected ExtendedLabel jLabel_FileName = null;  //  @jve:decl-index=0:
...
//			gridBagConstraints18.anchor = GridBagConstraints.CENTER;
			...
			jPanel_ButtonArea.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
			...
			jLabel_Title = new TitleLabel("tokutei.importdata.frame.title","tokutei.importdata.frame.guidance");
			jLabel_Title.setFont(new Font("ＭＳ ゴシック", Font.PLAIN, 14));

			// add s.inoue 2013/02/26 ?エラーの原因
			...
			jPanel_ButtonArea.add(getJPanel_TitleArea(), gridBagConstraints15);
		}
...
//			jButton_Import.addActionListener(this);
			...
			ImageIcon icon = iIcon.setStrechIcon(this, JPath.CONST_FIX_ICON);
			jButton_Import= new ExtendedButton(
					"Import","取り込み(I)","取り込み(ALT+I)",KeyEvent.VK_I,icon);
			jButton_Import.addActionListener(this);
		}
...
//			jButton_OpenFile.setMnemonic(KeyEvent.VK_O);
			...
			jButton_OpenFile= new ExtendedButton(
					"Reference","参照(O)","参照(ALT+O)",KeyEvent.VK_O,icon);
			jButton_OpenFile.addActionListener(this);
		}
